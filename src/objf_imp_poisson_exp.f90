module poisson_exp_imp
	use mvnorm
	use hmc
	use poisson_exp
    use imputation
	implicit none
	type, extends(poisson) :: poisson_imp
	type(impinfo) :: imp
	...
	double precision function objf_imp(this, nr, nc, X, y, np, p, nhp, hp) result(r)
		implicit none
		class(poisson_imp), intent(inout) :: this
		...
		! 線形回帰の目的関数を呼ぶ
		r = r + this%poisson%objf(nr, this%nev, X, y, this%nev, p, this%nev + this%nev**2, hp)
	...
	subroutine predict_imp(this, nr, nc, X, ss, np, P, y)
		...
		do i = 1, ss
			...
			call this%poisson%predict(nr, this%nev, X, 1, this%nev, P(i, :), y(1 + (i - 1)*nr : i*nr))
		end do
